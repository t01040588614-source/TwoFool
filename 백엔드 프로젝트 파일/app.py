...
from openai_recommender import enhance_passenger_recommendations, is_openai_enabled
from passenger_ai_chat import build_chat_reply, chat_mode_label, is_openai_key_configured
...
def ai_search_page():
    ...
    query = []
    if departure:
        query.append(f"departure={departure}")
    if arrival:
        query.append(f"arrival={arrival}")
    suffix = f"?{'&'.join(query)}" if query else ""
    return redirect(f"/{suffix}#passenger-ai-search")
...
def normalize_active_fault_trains(max_active=None):
    ...
    for location in TrainLocation.query.filter(
        TrainLocation.operation_status.in_(("delayed", "stopped", "disrupted")),
...
def _update_train_location(location):
    ...
    is_in_service = schedule.departure_time <= current_time < schedule.arrival_time
    is_waiting_departure = current_time < schedule.departure_time
    if is_in_service:
        map_progress = schedule_progress
    else:
        duration_ms = max(int(total_seconds * 1000), 60_000)
        offset_ms = (abs(hash(train.id)) * 7919) % duration_ms
        elapsed_ms = (int(current_time.timestamp() * 1000) + offset_ms) % duration_ms
        map_progress = elapsed_ms / duration_ms
    ...
    if not is_in_service:
        remaining_minutes = max(
            int((1.0 - map_progress) * (total_seconds // 60)), 1)
    ...
    elif is_in_service or map_progress > 0.0:
...
def _upsert_direct_train_pair(
    ...
    *,
    ...
    offset_minutes = (abs(hash(train_number)) % (7 * 24 * 60)) + 12
...
def seed_direct_national_trains():
    """전국 모든 시(역) 간 직통 고속 편성을 DB에 준비합니다."""
    if app.config.get("TESTING") or os.environ.get("PYTEST_CURRENT_TEST"):
        return
...
def build_passenger_control_trust(status_counts, prediction_summary):
    total = sum(int(status_counts.get(key, 0) or 0) for key in status_counts)
...
def build_dashboard_summary():
    locations = TrainLocation.query.all()
    status_counts = {"normal": 0, "delayed": 0,
                     "stopped": 0, "disrupted": 0, "arrived": 0}
    for location in locations:
        status = location.operation_status if location.operation_status in status_counts else "normal"
...
def scmaglev_dashboard_events():
    ...
    if event_group == "control":
        query = query.filter(OperationEventLog.event_type.like("CONTROL_%"))
    elif event_group == "alarm":
        query = query.filter(OperationEventLog.event_type.like("ALARM%"))
    elif event_group == "payment":
        query = query.filter(OperationEventLog.event_type.like("PAYMENT_%"))
    elif event_group == "vehicle":
        query = query.filter(OperationEventLog.event_type.like("VEHICLE_%"))
...
def scmaglev_ack_dashboard_events_bulk():
    ...
    limit = parse_int(data.get("limit", 100), 100, 1, 200)
...
def _build_ai_chat_context(departure_name: str, arrival_name: str) -> dict:
    now = now_utc_naive()
    public_summary = build_dashboard_summary()
    search_rows, _search_mode = _search_train_schedules(
        departure_name=departure_name,
        arrival_name=arrival_name,
        train_type_code="",
    ...
    for search_row in search_rows:
        ...
        if candidate:
            candidates.append(candidate)
    recommendations = build_passenger_recommendation_picks(candidates)
    recommendations, _ai_meta = enhance_passenger_recommendations(
        recommendations,
        {"departure": departure_name, "arrival": arrival_name},
    )
    return {
        ...
        "search_rows": search_rows[:8],
        "recommendations": recommendations,
    }


@app.route("/api/scmaglev/passenger/ai-chat/status", methods=["GET"])
def scmaglev_passenger_ai_chat_status():
    configured = is_openai_key_configured()
    meta = {
        "source": "openai" if configured and is_openai_enabled() else "rule_based",
        "openai_configured": configured,
        "openai_enabled": configured and is_openai_enabled(),
        "model": Config.OPENAI_MODEL,
    }
    return jsonify(
        {
            "mode_label": chat_mode_label(meta),
            "ai_meta": meta,
        }
    )


@app.route("/api/scmaglev/passenger/ai-chat", methods=["POST"])
def scmaglev_passenger_ai_chat():
    data = request.get_json(silent=True) or {}
    message = (data.get("message") or "").strip()
    if not message:
        return jsonify({"message": "질문을 입력해주세요."}), 400

    departure_name = (data.get("departure") or "").strip() or "서울"
    arrival_name = (data.get("arrival") or "").strip() or "수원"
    context = _build_ai_chat_context(departure_name, arrival_name)
    reply, ai_meta = build_chat_reply(
        message,
        departure_name,
        arrival_name,
        context,
    ...
    return jsonify(
        {
            "reply": reply,
            "ai_source": ai_meta.get("source", "rule_based"),
            "mode_label": chat_mode_label(ai_meta),
            "ai_meta": ai_meta,
            "route": {"departure": departure_name, "arrival": arrival_name},
        }
    )
